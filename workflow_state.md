...
## Current Phase
Deonysus AI Critic Agent (AWS Bedrock), Gating & Full App Visuals Complete.

## Current Task
- **Deonysus AI Agent (AWS Bedrock Integration, 20-Entry Unlock Gate & Playful Roasts)** — **COMPLETED**
  - **Screen Inventory & Spec**:
    - Updated `docs/design.md` with `[AI1] Deonysus (/deonysus)`.
    - Created `docs/screens/deonysus_deony/spec.md` preserving 1:1 documentation mapping per `RULE[AGENTS.md]`.
  - **AWS Bedrock Runtime & Responsible AI Guardrails**:
    - Installed `@aws-sdk/client-bedrock-runtime`.
    - Created `server/handlers/deonysus.ts` calling Bedrock `ConverseCommand` (`anthropic.claude-3-haiku` / Amazon Nova) with `guardrailConfig` (`deony-olympian-guardrail-v1`).
    - Implemented enterprise-grade multi-layer defense-in-depth aligned with AWS Certified AI Practitioner (AIF-C01):
      1. **Prompt Injection & Jailbreak Defense**: Pre-inference deterministic regex intercepting instruction overrides ("ignore previous instructions", "DAN", system prompt extraction) with playful Olympian rebuke before model invocation.
      2. **PII Masking & Redaction**: Real-time regex sanitizing credit card numbers, SSNs, emails, and phone numbers (`[REDACTED_PII]`).
      3. **Content Filtering & Safety**: Blocks self-harm, severe toxicity, and out-of-domain requests.
      4. **Contextual Grounding**: Grounds roasts exclusively in user-owned DynamoDB library records to mitigate hallucination.
      5. **Observability & Telemetry**: Returns latency, guardrail status (`PASSED` vs `INTERVENED`), and PII redaction flags.
    - Added resilient local Dionysus roast generator fallback when running offline or without active Bedrock credentials.
    - Mounted `POST /ai/deonysus` with Cognito authentication middleware in `server/routes.ts`.
  - **Archiver Threshold (20-Entry Gate)**:
    - If user has `< 20` logged experiences:
      - Sidebar renders dimmed, disabled state with lock icon and `Deonysus (X/20)` counter.
      - Visiting `/deonysus` directly displays the Temple Gate locked screen with an animated progress bar (`X / 20 offerings`).
    - If user has `>= 20` logged experiences:
      - Sidebar renders active link with wine goblet icon (`wine_bar`) and `AI` badge.
      - Accesses the full Deonysus Chatbot Sanctuary.
  - **Playful Dionysian Preset Commands & UI**:
    - `🔥 /roast`: Full Olympian critique of user's habits and conflicting tastes.
    - `✨ /taste`: Contrasts 5-star masterpieces with questionable low-rated guilty pleasures.
    - `📜 /backlog`: Mocks abandoned wishlist items and in-progress items gathering dust.
    - `🍇 /grape`: Dionysian mythological easter egg: Bacchus lore, wine goblets, Athenian theatre festivals, and shade thrown at Apollo.
    - "Guardrails Active" header badge with full interactive AWS AI Practitioner Architecture modal.
    - Freeform interactive chat input with fresh session support on each visit.
  - **Testing & Verification**:
    - Vitest unit tests: 48/48 passing (`server/handlers/__tests__/deonysus.test.ts` testing 401 unauthenticated, 403 < 20 gate, /roast, /taste, /backlog, /grape lore, Bedrock live dispatch, prompt injection defense, and PII redaction).
    - Production build: `npm run build` compiled cleanly in 1.04s.
    - Captured 15 full-page screenshots in `screenshots-deonysus/` via Playwright:
      - `01-sidebar-locked-deonysus.png`
      - `02-deonysus-locked-temple-gate.png`
      - `03-sidebar-unlocked-deonysus.png`
      - `04-deonysus-chat-welcome.png`
      - `04b-deonysus-guardrails-architecture-modal.png`
      - `04c-deonysus-prompt-injection-blocked.png`
      - `05-deonysus-command-roast.png`
      - `06-deonysus-command-taste.png`
      - `07-deonysus-command-backlog.png`
      - `08-deonysus-command-grape-easteregg.png`
      - `09-library-catalog.png`
      - `10-library-insights-stats.png`
      - `11-timeline-view.png`
      - `12-settings-sanctuary.png`
      - `13-landing-page.png`
  - **Production Deployment to AWS**:
    - Live URL: `https://d1cdomhzh1pe4j.cloudfront.net`
    - CDK: Deployed updated `DeonyStack` with Bedrock IAM policies (`bedrock:InvokeModel`, `bedrock:ApplyGuardrail`).
    - Backend: Lambda `deony-api-handler` bundled with Deonysus agent and guardrails handler.
    - S3: `dist/` synced to `deonystack-frontendbucketefe2e19c-xidikzenbqoy`.
    - CloudFront: Invalidation `I7DAHK3DMCHFO5EEJQ1KEZFVML` submitted on `E2FBFV25USKQU1`.
    - Git: Committed and pushed to `main` (`b6800a8` / `b2d375d`).
  - **Comprehensive Security Audit & Vulnerability Remediation Complete**:
    - **Global Media Invariant Hardening (`server/handlers/media.ts`)**: Fixed critical authorization bypass in `updateMedia` where unowned/provider media could be overwritten. Enforced that only user-owned manual media (`is_manual: true`, `user_id === callerId`) can be edited; provider items reject edits with 403.
    - **User Privacy & IDOR Masking (`server/handlers/user.ts`)**: `getUser` (`GET /users/:id`) now enforces profile privacy, masking non-owner private profiles identically to `getUserByUsername`. `createUser` now requires authentication (`authMiddleware`) and binds to Cognito `sub`.
    - **S3 Path Traversal Prevention (`server/handlers/upload.ts`)**: Strict alphanumeric regex validation on `media_id` prevents directory traversal (`../`). Restricted `content_type` to allowed image MIME types (`jpeg`, `png`, `webp`). Shortened presigned URL TTL to 15 minutes.
    - **Cryptographic JWT Validation (`server/middleware/auth.ts`)**: Enforced strict RS256 signature verification, issuer checking against Cognito User Pool, audience/client_id verification, and `token_use` claims (`id` | `access`). Added fail-closed enforcement when Cognito is unconfigured in production.
    - **Rate Limiting & Abuse Prevention (`server/routes.ts`)**: Added tiered `express-rate-limit` middleware across all routes (general API: 600/15m, AI Deonysus: 25/min, media search: 60/min, S3 upload: 50/15m).
    - **Bedrock Input Bounds & Exfiltration Defense (`server/handlers/deonysus.ts`)**: Capped input length to max 1,000 characters to prevent ReDoS and Bedrock token exhaustion. Added regex detection for markdown image exfiltration and hidden prompt extraction attempts.
    - **Batch API & Search Bounds (`server/handlers/media.ts`, `server/handlers/search.ts`)**: Capped `batchGetMedia` to 100 items to eliminate DynamoDB validation exceptions. Bounded search query `q` to 200 characters.
